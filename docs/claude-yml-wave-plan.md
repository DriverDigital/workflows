...
| **Macroscope CLI in the run** | A setup step installs the CLI with the Claude Code plugin and the issue prompt runs `/macroscope:autoloop` before `gh pr create` — Macroscope reviewing in-run, in the slot the Claude pre-review held until 2026-09-12. | A non-interactive credential. The installer takes `--tools claude --yes`, but auth is a browser wizard under `~/.macroscope` and reviews bill agent credits; nothing in the docs or installer offers a token path. Ask Macroscope; same shape as the Figma blocker until answered. |
...
| **Pre-review and reviewer request removed** | Already in the kit (2026-09-12): the issue prompt no longer runs `/code-review high` before the PR nor `gh pr edit --add-reviewer`, and quality-standard item (4) now says not to self-review — Macroscope owns review ([`macroscope-integration-scope.md`](macroscope-integration-scope.md)). | None. Rides the next tag; cut one for it alone if the fleet should stop double-reviewing before another ride-along clears (v1.7.0–v1.9.0 were kit-only tags). |
